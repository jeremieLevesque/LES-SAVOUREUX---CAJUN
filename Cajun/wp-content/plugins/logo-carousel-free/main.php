 ...
 * Plugin URI:        https://shapedplugin.com/logo-carousel/?ref=1
	...
	class SP_Logo_Carousel {
		...
		public function public_scripts() {
			...
			if ( $found_generator_id ) {
				...
				wp_add_inline_style( 'sp-lc-style', $dynamic_css );
			}
		...
		public function delete_page_lcp_option_on_save( $post_ID ) {
			if ( is_multisite() ) {
				...
				if ( delete_option( 'sp_lcp_page_id' . $post_ID ) ) {
		...
		public function add_plugin_action_links( $links, $file ) {
			...
			if ( SP_LC_BASENAME === $file ) {
				...
				$links['go_pro'] = sprintf( '<a href="%s" style="%s">%s</a>', 'https://shapedplugin.com/logo-carousel/pricing/?ref=1', 'color:#1dab87;font-weight:bold', __( 'Go Pro!', 'logo-carousel-free' ) );
			}
		...
		public function after_logo_carousel_row_meta( $plugin_meta, $file ) {
			if ( SP_LC_BASENAME === $file ) {
				$plugin_meta[] = '<a href="https://shapedplugin.com/logo-carousel/logo-carousel-lite-version-demos/" target="_blank">' . __( 'Live Demo', 'logo-carousel-free' ) . '</a>';
			}
			return $plugin_meta;
		}
